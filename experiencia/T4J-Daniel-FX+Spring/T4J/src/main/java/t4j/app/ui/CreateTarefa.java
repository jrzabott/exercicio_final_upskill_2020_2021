...
public class CreateTarefa implements Initializable {
    ...
    private void btnRegistarTarefa(ActionEvent event) {
        try {
            ...
            if (mainSceneController.getUidto().getEmail() == null || mainSceneController.getUidto().getEmail().isEmpty()) {
                throw new ElementoInvalidoException("Para criar uma tarefa é necessário que haja um colaborador logado.");
            }
