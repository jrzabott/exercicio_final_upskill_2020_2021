...
public class CreateFreelancer implements Initializable {
    ...
    @FXML
    private void btnRemoverNovaCompetenciaTecnica(ActionEvent event) {
        ReconhecimentoCTDTO selectedItem = table_ct.getSelectionModel().getSelectedItem();
        if (selectedItem != null) {
            reconhecimentos.remove(selectedItem);
        }
        table_ct.refresh();
    }
    @FXML
    private void btnRemoverNovaHabilitacaoAcademica(ActionEvent event) {
        HabilitacaoAcademicaDTO selectedItem = table_ha.getSelectionModel().getSelectedItem();
        if (selectedItem != null) {
            habilitacoes.remove(selectedItem);
        }
        table_ha.refresh();
    }
