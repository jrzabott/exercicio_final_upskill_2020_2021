...
//        this.APP_KEY = UIMainSceneController.getAPP_KEY();
//
//    }
    /**
     *
    ...
    public boolean login(UserInfoDTO u) {
        ...
        if (hr.getResponseCode() != HttpURLConnection.HTTP_OK) {
            ...
            try {
                readValue = new ObjectMapper().readValue(hr.getResponseBody(), Object.class);
                ArrayList arrayList = (ArrayList<LinkedHashMap>) readValue;
                lhm = (LinkedHashMap) arrayList.get(0);
                error.append("Erro ao realizar login. ");
                error.append("Http Code: ").append(hr.getResponseCode());
                error.append(". Mensagem: ");
                if (lhm != null) {
                    error.append((String) lhm.get("error"));
                }
                throw new UsersAPIException(error.toString());
            } catch (JsonProcessingException ex) {
                // System.out.println("UsersAPIAdapter.registerUserWithRoles - Erro ao processar JSON.");
            }
        }
        return false;
    }
    ...
    public boolean registerUserWithRoles(UserInfoDTO u) {
        ...
        if (hr.getResponseCode() != HttpURLConnection.HTTP_OK && hr.getResponseCode() != HttpURLConnection.HTTP_CREATED) {
            ...
            try {
                ...
                ArrayList arrayList = (ArrayList<LinkedHashMap>) readValue;
                lhm = (LinkedHashMap) arrayList.get(0);
                error.append("Erro ao registar utilizador. ");
                error.append(". Http Code: ").append(hr.getResponseCode());
                error.append(". Mensagem: ");
                if (lhm != null) {
                    error.append((String) lhm.get("error"));
        ...
        return (hr.getResponseCode() == HttpURLConnection.HTTP_OK || hr.getResponseCode() == HttpURLConnection.HTTP_CREATED);
    }
    ...
    private String getNewAppContext() {
        ...
        try {
            ...
            throw new LoginServerException("Falha no formato do JSON do contexto. " + e.getMessage());
